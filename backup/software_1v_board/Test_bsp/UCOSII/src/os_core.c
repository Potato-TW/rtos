...
*                                            APPEND MESSAGE
*
* Description: This function appends a message to the message list for logging context switches.
*
* Arguments  : currTime_    is the current time when the event occurred
*              event_       is the type of event (PREEMPT_IDX or COMPLETE_IDX)
*              fromTaskId_  is the ID of the task being preempted or completed
*              toTaskId_    is the ID of the task taking over
*
* Returns    : none
*********************************************************************************************************
*/
...
{
    // printf("append");
    msgBuf[msgBufTail].currTime = currTime_;
    msgBuf[msgBufTail].event = event_;
    msgBuf[msgBufTail].fromTaskId = fromTaskId_;
    msgBuf[msgBufTail].toTaskId = toTaskId_;

    msgBufTail++;
    if (msgBufTail >= MSG_BUF_SIZE)
    {
        msgBufTail = 0;
    }
}